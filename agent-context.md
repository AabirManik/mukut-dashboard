...
| `test/v5_rangecal_test.js` | unit | v5 two-point range calibration: fit math (path-loss inversion), persistence, bridge fitted override, ML bypass when calibrated, helmet offset auto-disable, restore/clear gate, full pipeline (26 assertions) |
| `test/v6_tracking_test.js` | unit | v6 session tracking + proximity pull: START/STOP lifecycle (freeze path, resume with odometer resync), proximity mismatch-gated pull (accurate anchor untouched, mismatch pulled toward node), motion-agnostic path growth (beacon-only movement accumulates), adaptive correction strength k=0.35 when delta=0, anchor EMA step detection (re-seed on genuine move), StateManager integration (session/proximity in state, global disable gate) (32 assertions) |
...
No CI, no coverage tooling. `npm test` runs only `phase1_test.js` (Gotcha G6). All 14 unit/integration suites (phase 1/2/4/5/6, ml_test, filter_validation, hardware_validation, v2_bridge_check, v3_routemap_test, v4_mapcal_test, v5_rangecal_test, v6_tracking_test) are 100% green as of 2026-09-13; the E2E suites are deferred to the hardware-integration phase.
...
- **G37 — v5 two-point range calibration (RANGE CAL):** `rangeCalibrator.js` fits a real path-loss curve from two helmet placements (1 m and 3 m from a known node). The fitted curve (`A`, `n` parameters) replaces path-loss/ML as the primary distance source when calibrated; `calibrationEngine.displayDistance()` returns fitted distances first. Helmet display offsets (`applyHelmetOffset`) auto-disable when fitted curve is active. Persistence to `config/range_calibration.json` (override: `range_calibration.file`); restored on boot ONLY in live hardware modes (`persistentLiveMode` — G38). Simulators and tests never inherit live calibrations.
- **G38 — Persisted calibration restoration is gated on persistentLiveMode:** map geometry (`config/map_geometry.json`) and range calibration (`config/range_calibration.json`) are ONLY restored when `persistentLiveMode` is truthy (i.e. `data_source` is `node1|node1_wifi|hardware|serial`). Simulator sessions and unit tests start clean every time. `trajectoryEngine.clearCalibration()` deletes both files. This prevents stale live-session calibrations from leaking into test assertions.
- **G39 — Test isolation for live-mode StateManagers:** all test suites that construct `new StateManager()` in live-hardware mode MUST use `isolatedConfig()` (or `isolate(config)`) from `test/helpers/isolated_config.js` — this redirects `trajectory.map_geometry_file` and `range_calibration.file` to unique temp locations, preventing stale user calibrations from breaking deterministic assertions. `phase5_test.js` is the only suite that skips this (simulator mode, calibration never restored). The fix was applied across all 12 affected suites in the v6 session.
- **G40 — Tracking session lifecycle (v6):** `startTracking` clears breadcrumb + re-fixes position from beacons; `stopTracking` freezes trajectory (heading capture, odometer baselines, map calibration keep running for resume). Default ON for always-on demo behavior. `POST /api/tracking` with `{action: 'start'|'stop'}`. State exposes `route_map.session.active` + `started_at`.
- **G41 — Proximity pull is mismatch-gated (v6):** when the strongest beacon is within `proximity_radius_m` (5 m) and the fused position DISAGREES with the measured range (mismatch > `proximity_mismatch_m` = 1 m), the position is pulled toward the node by up to `proximity_max_pull` (0.65 m). Accurate trilateration (consistent anchors) is never touched. The pull only fires on engage transition (de-duped via `lastProximityId`).
- **G42 — Adaptive correction strength (v6):** `correctionStrengthAgnostic` (0.35) is applied when `delta === 0` (no step displacement) — more assertive anchor correction when beacons are the sole motion signal. The resync branch (k=0.5) still overrides for large offsets.
- **G43 — Anchor EMA step detection (v6):** range jumps > 3 m or > 50% of the previous value re-seed the per-anchor EMA instantly (genuine movement). Jitter below the threshold is smoothed at α=0.35. This prevents stale EMA history from lagging behind real movement.

---

## 17. Current Status & Next Steps

- Software Phases 1–13 complete and regression-tested (schema/safety, network, routing/failover, hardware ingestion, route mapping, distance calibration, ML distance estimation, temporal median filter, v2 network truth, v3 ML-driven route mapping, v4 dynamic map calibration, v5 two-point range calibration, v6 session tracking + proximity pull).
- **Phase 13 (2026-09-13): v6 session tracking + proximity pull** — START/STOP TRACKING controls the route-map session (clears breadcrumb + re-fixes on start, freezes trajectory on stop; heading/odometry baselines/map cal keep running for seamless resume); mismatch-gated proximity pull snaps the position toward the strongest beacon when the map disagrees with the measured range; motion-agnostic path growth lets the route draw from beacon signals alone (no steps needed); adaptive correction k=0.35 when displacement=0; anchor EMA step detection re-seeds on genuine movement. `test/v6_tracking_test.js` 32/32; all 14 suites 100% green. Test isolation fix: all live-mode `new StateManager()` in tests now use `isolatedConfig()` to prevent stale calibrations from breaking assertions.
- **Phase 12 (2026-09-13): v5 two-point range calibration** — `rangeCalibrator.js` fits a real path-loss curve from two helmet placements; fitted distances become the primary distance source when calibrated; ML bypassed; helmet offsets auto-disabled; persisted to `config/range_calibration.json`, restored ONLY in live hardware modes. `test/v5_rangecal_test.js` 26/26; all 13 suites 100% green.
- **ONNX model retrained (v2):** new Random Forest trained on Salvora outdoor LoRa GPS dataset (2,647 rows) + MUKUT hardware (60 rows); model at `models/mukut_distance_model.onnx` (1.2 MB); range 3–6656 m; v5 fitted curve remains primary distance source when calibrated; proximity pull fixes the close-range accuracy issue. Training data in `training/lora_training_dataset.csv` (2,707 rows).
